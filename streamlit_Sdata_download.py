...
if authentication_status:
    ...
    option = st.sidebar.selectbox('Select Analysis',('DC Power Time Series', 'Energy Production', 'Current Voltage Time Series', 'Temperature Coefficient of Voltage'))
...
#                 g = sns.JointGrid(x="Tmod", y="V_norm", data=v_df, marginal_ticks=True)
#                 g.plot_joint(plt.scatter, c=v_df.G, label='$V$$_{normalized}$', cmap="Blues", s=7)
#                 cbar = plt.colorbar()
#                 cbar.set_label('Irradiance $[W/m^2]$', size=5)
#                 cbar.ax.tick_params(labelsize=5) 
#                 g.plot_marginals(sns.histplot, kde=True, color="steelblue")
#                 g.fig.set_dpi(200)

#                 f1 = v_filter[v_filter.V_coef > 0]
#                 g.x = f1.Tmod
#                 g.y = f1.V_coef
#                 g.plot_joint(plt.scatter, c='orange', s=20, alpha=0.5, edgecolor="white", label="$V$$_{filtered}$")
#                 g.plot_marginals(sns.histplot, kde=True, color="orange")

#                 g.x = f1.Tmod
#                 g.y = f1.V_reg
#                 g.plot_joint(plt.plot, c='red', label="$V$$_{regression}$ \n  Coef$_{calculated}$ ($V$$_{mpp}$) = %.2f [%%$/°C$] \n  Coef$_{reported}$ ($V$$_{oc}$) = %.2f [%%$/°C$]" %(float(v_coef), c_voltage))

#                 g.ax_joint.legend(loc='upper center', bbox_to_anchor=(0.7, 1.38), ncol=3, fancybox=True)
#                 g.ax_joint.set_xlabel('Module temperature $[°C]$')
#                 g.ax_joint.set_ylabel('Normalized Voltage [%]')
                
                # fig = px.scatter(v_filter, x="Tmod", y=["V_norm","V_coef"],labels={"Tmod": "PV Module Temperature (°C)"}, height=600)
                # fig.update_traces(marker=dict(size=8))
                # fig = px.scatter(v_filter, x="Tmod", y="V_norm",labels={"Tmod": "PV Module Temperature (°C)"}, height=600, color = "V_norm")
                fig = make_subplots()
                fig.add_trace(go.Scatter(x=v_filter["Tmod"],y=v_filter["V_norm"],name="All Values", mode ="markers", marker={'symbol': 'x', 'size': 3,'color':"orange"}))
                fig.add_trace(go.Scatter(x=v_filter["Tmod"],y=v_filter["V_coef"],name="Filtered Values", mode ="markers", marker={'size': 6,'color':"blue"}))
