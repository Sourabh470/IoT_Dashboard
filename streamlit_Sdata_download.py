...
# st.write('<style>div.row-widget.stRadio >.element-container:nth-child(0) {left: -0px;top:  -0px;} div{flex-direction:row;left: -0px;top:  -200px;}</style>', unsafe_allow_html=True)
...
if authentication_status:
    ...
    option = st.sidebar.selectbox('Select Analysis',('DC Power Time Series', 'Energy Production', 'Current Voltage Time Series', 'Temperature Coefficient of Voltage', 'Degradation Analysis of Current and Voltage', 'Compare Plants'))
    ...
    if sensor_ID not in list(sensor_data.sensor_ID):
       ...
       # st.text(sensor_ID)
        ...
        # st.write(final_data)
        #condition = (final_data['datetime'] > start_date) & (final_data['datetime'] <= end_date)
        #dff = final_data[condition]
        ...
        if len(final_data) != 0:    
            ...
            if option == 'Energy Production':
                ...
                if timedelta(days=31) > (end_date - start_date) >= timedelta(days=1):
                    dff = dff.groupby(pd.Grouper(freq='d')).sum()
                elif timedelta(days=90) >= (end_date - start_date) >= timedelta(days=31):
                ...
                fig = px.bar(        
                        dff,
                        x = "datetime",
                        y = "Energy Yield(kWh)", height=600, labels={"datetime": "Date/Time","Energy Yield(kWh)": "Energy Yield (kWh)"},
               ...
               # fig.update_xaxes(tickformat="%d-%m-%y")
                st.markdown('<style>div.stPlotlyChart > div{flex-direction:row;justify-content: left;position:absolute;top: -25px;} </style>', unsafe_allow_html=True)
                st.plotly_chart(fig, use_container_width=True, config = {'displaylogo': False})
                
            elif option == 'DC Power Time Series':
                # dff = final_data[['datetime', 'Power(W)']].copy()
                fig = px.line(        
                        final_data,
                        x = "datetime",
                        y = "Power(W)", height=600, labels={"datetime": "Date/Time","Power(W)": "Power (W)"},
            ...
            elif option == 'Current Voltage Time Series':
                # dff = final_data[['datetime','Current(A)', 'Voltage(V)']].copy()              
                fig = make_subplots(specs=[[{"secondary_y": True}]])
                fig.add_trace(go.Scatter(x=final_data["datetime"],y=final_data["Current(A)"], mode ="lines", name="Current"),secondary_y=False,)
                fig.add_trace(go.Scatter(x=final_data["datetime"],y=final_data["Voltage(V)"], mode ="lines", name="Voltage"),secondary_y=True,)
                Inputs = File_object1.loc[File_object1['sensor_CCID']==sensor_ID]
                Impp = Inputs['I_mpp'].tolist()[0]
                Vmpp = Inputs['V_mpp'].tolist()[0]
                fig.add_hline(y=Impp, line_dash="dot", annotation_text="Impp", annotation_position="bottom left", secondary_y=False)
                fig.add_hline(y=Vmpp, line_dash="dot", annotation_text="Vmpp", annotation_position="top left", secondary_y=True)
                ...
                sensor_data1 = sensor_data.copy()
                sensor_data1.rename(columns={'Mod. Temp. (°C)':'Tmod', 'Voltage(V)':'V', 'Current(I)':'I', 'Power(W)':'P'}, inplace = True)
...
#                 st.write(Inputs)
                ...
                solpos = solarposition.get_solarposition(time=sensor_data1['datetime'],
                                           latitude=lat,
                                           longitude=lon)
#                 st.write(solpos)
                AM = site.get_airmass(times=None, solar_position=solpos, model='kastenyoung1989')
#                 st.write(AM)
                sensor_data1['G'] = 1000*(1 + ((sensor_data1['I'] - Impp)- (alpha/100)*(sensor_data1['Tmod'] - 25))/Isc)
#                 st.write(sensor_data1.dropna().describe())
                sensor_data1['V_norm'] = sensor_data1['V']*100/Vmpp
                v_df = sensor_data1.copy()
#                 st.write(v_df)
                AM_filter = AM[(AM['airmass_absolute']>=1.4) & (AM['airmass_absolute']<=1.6)]
#                 st.write(AM_filter)
                v_filter = v_df[v_df['datetime'].isin(AM_filter.index)]
#                 st.write(v_filter)
...
#                 st.write(v_filter)
                ...
                v_filter["V_reg"], v_coef, v_inter = get_tcoef(df_in=v_filter,
                                                                  ...
                                                                  temp_column="Tmod")
#                 st.write(v_filter)
                temp = v_filter[['Tmod', 'V_coef']].dropna()
                lr = LinearRegression()
                v_res = lr.fit(np.array(temp["Tmod"]).reshape(-1,1), temp['V_coef'])
                v_fit = lr.predict(np.array(temp["Tmod"]).reshape(-1,1))
                fig = make_subplots()
                fig.add_trace(go.Scatter(x=v_filter["Tmod"],y=v_filter["V_norm"],name="All Values", mode ="markers", marker={'symbol': 'x', 'size': 3,}))
                fig.add_trace(go.Scatter(x=v_filter["Tmod"],y=v_filter["V_coef"],name="Filtered Values", mode ="markers", marker={'size': 6,}))
                fig.add_trace(go.Scatter(x=temp["Tmod"], y=v_fit, mode="lines", name='Voltage Fit'))
#                 fig = px.scatter(v_filter, x="Tmod", y="V_coef",labels={"Tmod": "PV Module Temperature (°C)"}, height=600, trendline="ols")
                fig.update_layout(title= "Temperature coefficient of Voltage<br><sup>Calculated after filtering measured voltage by device</sup>", title_x=0.5,title_y=0.95)
                fig.update_yaxes(range = [50,100], title_text= "Normalized Vmpp (V)")
                fig.update_xaxes(title_text= "Module Temperature (°C)")
                fig['layout']['title']['font'] = dict(size=20)
                fig.update_layout(yaxis = dict(tickfont = dict(size=14), titlefont = dict(size = 16)))
                fig.update_layout(xaxis = dict(tickfont = dict(size=14), titlefont = dict(size = 16)))
                fig.update_layout(legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=0.95))
                ...
                sensor_data2.rename(columns={'Mod. Temp. (°C)':'Tmod', 'Voltage(V)':'V', 'Current(I)':'I', 'Power(W)':'P'}, inplace = True)
...
#                 st.write(sensor_data_comp.columns)
                ...
                Impp = Inputs['I_mpp'].tolist()[0]
                Vmpp = Inputs['V_mpp'].tolist()[0]
                sensor_data2['G'] = 1000*(1 + ((sensor_data2['I'] - Impp)- (alpha/100)*(sensor_data2['Tmod'] - 25))/Isc)
                mfoc = sensor_data2.copy()
                mfoc['I_norm'] = mfoc['I']/Impp
                mfoc['V_norm'] = mfoc['V']/Vmpp
                mfoc['month'] = mfoc['datetime'].dt.month
#                 st.write(mfoc)
                mfoc_filter = mfoc[((mfoc['G']>790) & (mfoc['G']<810)) & ((mfoc['Tmod']>53) & (mfoc['Tmod']<57))].copy()
                temp_df = mfoc_filter.groupby('month').mean()
#                 st.write(temp_df)
#                 from sklearn.linear_model import LinearRegression
                lr = LinearRegression()
                v_res = lr.fit(np.array(temp_df.index).reshape(-1,1), temp_df['V_norm'])
                v_fit = lr.predict(np.array(temp_df.index).reshape(-1,1))
                i_res = lr.fit(np.array(temp_df.index).reshape(-1,1), temp_df['I_norm'])
                i_fit = lr.predict(np.array(temp_df.index).reshape(-1,1))
                fig = make_subplots(rows=2, cols=1, vertical_spacing=0.05)
                fig.add_trace(go.Scatter(x=temp_df.index, y=temp_df['V_norm'], mode='markers', name='Voltage'), row=1, col=1)
                fig.add_trace(go.Scatter(x=temp_df.index, y=v_fit, mode="lines", name='Voltage Fit'), row=1, col=1)
                fig.add_trace(go.Scatter(x=temp_df.index, y=temp_df['I_norm'], mode='markers', name='Current'), row=2, col=1)
                fig.add_trace(go.Scatter(x=temp_df.index, y=i_fit, mode="lines", name='Current fit'), row=2, col=1)
                fig.update_layout(height=700, width=800, title_text="Side By Side Subplots")
                fig.update_layout(title= "Degradation analysis of Current & Voltage<br><sup>Horizontal line indicates no degradation </sup>", title_x=0.5,title_y=0.93)
                fig.update_yaxes(range = [0.5,1.0], title_text= "Normalized Value")
                fig.update_xaxes(title_text= "Month", row=2, col=1)
                fig['layout']['title']['font'] = dict(size=20)
                st.markdown('<style>div.stPlotlyChart > div{flex-direction:row;justify-content: left;position:absolute;top: -40px;} </style>', unsafe_allow_html=True)
                st.plotly_chart(fig, use_container_width=True, config = {'displaylogo': False})
            
            elif option == 'Compare Plants':
                sensor_data_global = pd.DataFrame()
                for i in range (len(CCID)):
                    sensor_data_temp = connecting_to_server(CCID[i])
                    sensor_data_global = pd.concat([sensor_data_global, sensor_data_temp], ignore_index=True)
                    condition1 = (sensor_data_global['datetime'] > start_date) & (sensor_data_global['datetime'] <= end_date)
                dff = sensor_data_global[condition1]
                # st.write(dff)
                Parameters = st.radio('Select Parameters',('DC Power Time Series', 'Energy Production', 'Temperature Coefficient of Voltage', 'Degradation Analysis of Current and Voltage'))
                st.markdown('<style>div.row-widget.stRadio > div{flex-direction:row;justify-content: left;position:absolute;top:  40px;} </style>', unsafe_allow_html=True)
                # st.write('Hello world', Parameters=='DC Power Time Series')
                if Parameters == 'DC Power Time Series':
                    fig = px.line(        
                        dff,
                        x = "datetime",
                        y = "Power(W)", color = "sensor_ID", height=600, labels={"datetime": "Date/Time","Power(W)": "Power (W)","sensor_ID": "Plan ID"},
                        title = "DC Power of PV Module<br><sup>Calculated as product of Current and Voltage</sup>"
                    )
                    ...
                    fig.update_layout(xaxis=dict(rangeslider=dict(visible=True)))
                    fig.update_xaxes(rangeslider_thickness = 0.05)
                    fig.update_layout(legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=0.98))
                    st.markdown('<style>div.stPlotlyChart > div{flex-direction:row;justify-content: left;position:absolute;top:  -160px;} </style>', unsafe_allow_html=True)
                    st.plotly_chart(fig, use_container_width=True, config = {'displaylogo': False})
                
                elif Parameters == 'Energy Production':
                    for i in range (len(CCID)):
                        dff_temp = sensor_data_global[sensor_data_global['sensor_ID'] == CCID[i]]
                        daily_yield_temp, total_yield_temp, output_energy_temp = energy_yield(dff_temp['Power(W)'])
                        dff_temp2 = pd.concat([dff_temp,output_energy_temp], axis = 1)   
                        dff_temp2.columns = ['Mod Temp(°C)','Current(A)','Voltage(V)','Power(W)', 'Energy Yield(kWh)']
                    
                    dff3 = dff2[['datetime', 'Energy Yield(kWh)']].copy()
                    dff3['Energy Yield(kWh)'] = dff['Energy Yield(kWh)']/1000
                    dff3.set_index('datetime', inplace = True)
                    if timedelta(days=31) > (end_date - start_date) >= timedelta(days=1):
                        dff3 = dff3.groupby(pd.Grouper(freq='d')).sum()
                    elif timedelta(days=90) >= (end_date - start_date) >= timedelta(days=31):
                        dff3 = dff3.groupby(pd.Grouper(freq='W')).sum()
                    elif (end_date - start_date) == timedelta(days=0):
                        dff3 = dff3.groupby(pd.Grouper(freq='H')).sum()
                    elif (end_date - start_date) > timedelta(days=90):
                        dff3 = dff3.groupby(pd.Grouper(freq='M')).sum()
                    dff3.reset_index(inplace = True)
                    fig = px.bar(        
                            dff3,
                            x = "datetime",
                            y = "Energy Yield(kWh)", color = "sensor_ID", height=600, labels={"datetime": "Date/Time","Energy Yield(kWh)": "Energy Yield (kWh)"},
                            title = "Energy Generated by PV module<br><sup>Calculated as product of Power and Time</sup>"
                        )
                    fig.update_layout(title_x=0.5, title_y=0.9)
                    fig['layout']['title']['font'] = dict(size=20)
